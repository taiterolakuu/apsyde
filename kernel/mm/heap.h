...
/* Возвращает границы heap. Пока heap работает в identity mapping —
 * это физические адреса. После перехода на higher-half (этап B4)
 * функция вернёт виртуальные адреса — но там же поменяется и вся
 * логика heap, так что API не изменится. */
